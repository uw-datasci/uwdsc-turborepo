...
## Real-World Example: Schedule API

Here's a complete implementation from the CXC app showing a **public API endpoint** that requires no authentication:

### API Route

```typescript
// apps/cxc/app/api/schedule/route.ts
...
```tsx
// apps/cxc/app/schedule/page.tsx
"use client";
...
export default function SchedulePage() {
  ...
  if (!events || events.length === 0) return <div>No events scheduled</div>;
...
- Add JSDoc comments
- Write unit tests
- Sort/filter data in API routes when appropriate
...
- Over-fetch data (return only what's needed)
