...
**Terms Table** (Web Package)

The Terms table manages application periods with flexible deadline support:
...
model Terms {
  ...
  app_soft_deadline DateTime @db.Timestamptz(6)  // Recommended deadline
  app_hard_deadline DateTime @db.Timestamptz(6)  // Final cutoff
...
**Key Features:**
- **Dual Deadline System**: Supports both soft (recommended) and hard (final) deadlines
- **Timezone-Aware**: Uses `Timestamptz` for global applicant support
- **Flexible Workflows**: Enables grace periods and priority consideration
- **Relations**: Links to applications and dynamic questions per term
